...
const HomeScreen = () => {
  ...
  return (
    <div className={`flex flex-col h-full min-h-screen pb-24 transition-colors duration-300 ${isDarkMode ? 'bg-black text-white' : 'bg-[#F5F5F5] text-black'
      }`}>
      <div className="p-4">
        {/* Header */}
        ...
        {/* Search Bar */}
        <div className={`rounded-2xl p-3.5 flex items-center shadow-sm mb-6 border transition-colors ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-gray-100 text-black'
          }`}>
        ...
        <div
          ...
          className={`rounded-2xl p-4 flex items-center mb-6 shadow-sm cursor-pointer hover:shadow-md active:scale-[0.99] transition-all border ${isDarkMode
        ...
        <div className="mb-8">
          ...
          {popularPosts.length === 0 ? (
            <div
              ...
              className={`p-5 rounded-2xl border text-center transition-all cursor-pointer shadow-sm ${isDarkMode
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {popularPosts.map(post => (
                <div
                  ...
                  className={`p-4 rounded-2xl min-w-[170px] shadow-sm border shrink-0 cursor-pointer active:scale-95 transition-all ${isDarkMode
                ...
                >
                  ...
                  <div className={`flex items-center justify-between text-[9px] pt-1.5 border-t ${isDarkMode ? 'border-zinc-800 text-zinc-500' : 'border-gray-50 text-gray-400'
                    }`}>
        ...
        <div className="mb-6">
          ...
          <div className={`rounded-2xl shadow-sm border p-1 transition-colors ${isDarkMode ? 'bg-zinc-950 border-zinc-850' : 'bg-white border-gray-100'
            }`}>
            {newsEvents.map((item, idx) => (
              <div
                key={item.id}
                onClick={() => handleNewsClick(item.url)}
                className={`flex items-start p-3.5 cursor-pointer active:opacity-75 transition-all ${idx !== newsEvents.length - 1
              ...
              >
                ...
                <div className="flex-1 text-left pr-2">
                  <h4 className={`font-bold text-xs leading-tight mb-1 flex items-center gap-1 ${isDarkMode ? 'text-zinc-200' : 'text-gray-800'
                    }`}>
                    <span>{item.title}</span>
                    <FaLink size={8} className="text-zinc-500 shrink-0" />
                  </h4>
                  <p className={`text-[10px] leading-relaxed ${isDarkMode ? 'text-zinc-450' : 'text-gray-500 font-light'
                    }`}>{item.desc}</p>
                </div>
              </div>
            ))}
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className={`w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-scale-up border max-h-[85vh] overflow-y-auto transition-colors ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-white' : 'bg-white border-gray-200 text-black'
            }`}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className={`p-1.5 rounded-full transition-colors ${isDarkMode ? 'hover:bg-zinc-800 text-zinc-400' : 'hover:bg-gray-100 text-gray-500'
                  }`}
              >
                <FaTimes size={16} />
              </button>
            ...
            <form onSubmit={handleUpdateNews} className="space-y-6">
              {editNewsList.map((item, idx) => (
                <div key={item.id} className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800' : 'bg-gray-50 border-gray-200'
                  }`}>
                  ...
                  <div className="space-y-3 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full text-xs px-3 py-2 rounded-xl outline-none border transition-colors ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-gray-300'
                          }`}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className={`w-full text-xs px-3 py-2 rounded-xl outline-none border transition-colors resize-none ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-gray-300'
                          }`}
                      />
                    </div>
